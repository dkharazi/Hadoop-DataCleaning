...
public class DataClean2 {
		...
		private Text remEntryVal = new Text();
			...
			for (String str: dataEntries) {
				String dateStr = str.substring(0,7) + str.substring(11,13);
	...
	public static class IntSumReducer
		...
		private int[] daysPerMonth = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
			...
			for (int entryIndex = 1; entryIndex < valList.size(); entryIndex++) {
				Integer curEntryDay = Integer.parseInt(valList.get(entryIndex).substring(1,3));
				Integer prevEntryDay = Integer.parseInt(prevLine.substring(1,3));

				if (prevEntryDay + 1 == curEntryDay) {
					keyEmit.set(key);
					valEmit.set(key.toString().substring(0,7) + prevLine);
					...
					while (prevEntryDay != curEntryDay && prevEntryDay < numDaysPerMonth) {
						...
						keyEmit.set(key);
						valEmit.set(key.toString().substring(0,7) + "-" + prevDay + prevLine.substring(3));
						context.write(keyEmit, valEmit);
						prevEntryDay++;
					}
				}
				prevLine = valList.get(entryIndex);
			}
			
			// Fill in final day
			prevLine = key.toString().substring(0,7) + "-" + Integer.parseInt(prevLine.substring(1,3)) + prevLine.substring(3);
			keyEmit.set(key);
	...
	public static void main(String[] args) throws Exception {
                ...
                //
                job.setJarByClass(DataClean2.class);
